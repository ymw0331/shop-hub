...
const decrementQuantity = async ( cart ) =>
{
  try
  {
    ...
    console.log( 'buld updated', updated );
  ...
  try
  {
    ...
    const emailData = {
      ...
      html: `
        ...
        <p>Visit <a href="${ process.env.CLIENT_URL }/dashboard/user/o3rders">your dashboard</a> for more details</p>
      `,
    };

    try
    {
      await sgMail.send( emailData, function ( err, info )
      {
        if ( err )
        {
          console.log( "Email not send", err.message );
        } else
        {
